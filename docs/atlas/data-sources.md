...
Covers the numeric-data research behind The Atlas: which World Bank indicators exist, how to
call the API without tripping its traps, and what REST Countries offered (and why it isn't used).
Moved here from `docs/superpowers/research/country-numbers-catalog.md`; content unchanged except
this note and the one below.
...
> **Superseded, read before using this file:** this research recommended building on **REST
> Countries v5** (§"Recommended architecture" below). [`design.md`](./design.md), written the
> same day but after this file, overrode that choice — its §3.3 says plainly "REST Countries is
> dead — do not use it" and the shipped code agrees: there is no REST Countries call anywhere in
> `lib/atlas/`. Everything REST Countries would have supplied (ISO codes, flags, capitals,
> currencies, borders, languages) comes instead from a Wikidata-only join table,
> `lib/atlas/iso-countries.ts`, generated by `scripts/atlas/build-iso.mjs` — see
> [`build-pipeline.md`](./build-pipeline.md). Keep this file for the World Bank half, which the
> shipped `lib/atlas/sources/worldbank.ts` follows closely (its own doc comment cites this
> catalogue's §3.6-equivalent traps by name). Also note: the shipped indicator catalogue,
> `lib/atlas/indicators.ts`, has **151** entries, not the ~160 estimated in this file's
