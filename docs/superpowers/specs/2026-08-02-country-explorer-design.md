...
| **UN Comtrade preview** | exports and imports by partner, by product | `comtradeapi.un.org/public/v1/preview/C/A/HS?reporterCode={m49}&period={yr}&cmdCode=TOTAL&flowCode=X` |
...
|---|---|---|
| `lib/atlas/iso-countries.ts` | ~250 rows: ISO alpha-2, alpha-3, M49 numeric, English name, Wikidata Q-id, Wikipedia title | The join key for every source. Never changes. Also the M49 → name map that UN Comtrade needs, because Comtrade returns numeric codes with `null` names. |
...
| **Comtrade returns `partnerDesc: null`** | Join numeric M49 against the local ISO table |
| **Comtrade `reporterCode` is M49, not ISO numeric** — India is `699`, not `356` | Same local table |
...
lib/atlas/
  ...
  sources/
    ...
    comtrade.ts         partner trade + M49 join
