...
<Compare caption="The same schema, with and without the two flags. Both catch the same class of failure — a malformed argument or an invented field — before it reaches your handler, rather than after.">
  <Panel label="Naive" title="A plain string, an open object">

A `passengers` field typed as a bare string comes back as `"2"` or as `"two"`, and an object with no `additionalProperties` rule lets the model add a field like `urgency` that nothing in your handler expects.

  </Panel>
  <Panel label="What to do" title="strict: true, additionalProperties: false">

Sampling itself is constrained to the schema, so `passengers` is an int every time, and any field the model invents is rejected before your handler ever sees it.
...
**The field reality:** production systems that work reliably have, on average, between four and twelve tools loaded per task. Systems with thirty or more tools in context show measurably worse selection accuracy and higher costs, even when using tool-search mechanisms to filter the catalogue. The pattern that survives is: start with consolidated, task-shaped tools; reach for dynamic tool loading only when you have hundreds and usage logs prove a small working set.

<Bars
  caption="The number this lesson opened with: thirty tools, one per endpoint, against four built around the jobs the agent actually does. Every tool in the array is loaded into context on every single step, whichever count you shipped."
  unit="tools"
  axis="Tools loaded into context, per step"
  bars={[
    { label: 'One tool per endpoint', value: 30 },
    { label: 'Task-shaped tools', value: 4, subject: true },
  ]}
/>
...
<Anatomy
  caption="Three lines out of REFUND_TOOL, doing the enforcement this lesson has been arguing for: an enum instead of a string, a closed object, and a schema-valid example the model can pattern-match against."
  label="Inside REFUND_TOOL's input_schema"
  parts={[
    {
      text: '"reason": {"type": "string", "enum": ["duplicate", "fraudulent", "requested_by_customer"]},',
      name: 'An enum, not a string',
      note: 'Closes the set to the three values run_refund actually knows how to handle.',
    },
    { text: '\n"required": ["charge_id", "reason"],\n' },
    {
      text: '"additionalProperties": False,',
      name: 'The object, closed',
      note: 'No field the model invents — an "urgency" nobody defined — gets through.',
      subject: true,
    },
    {
      text: '\n\n"input_examples": [\n  {"charge_id": "ch_3PqR2xEXAMPLE", "reason": "requested_by_customer"},\n]',
      name: 'A schema-valid example',
      note: 'Roughly twenty to fifty tokens, cheap next to three wasted round trips.',
    },
  ]}
/>
