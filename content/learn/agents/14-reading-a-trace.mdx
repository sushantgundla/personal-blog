...
<Steps
  caption="The step-four example above, walked forward one step at a time. Click through and watch the check — 'given everything before it, is this step correct?' — stay yes until step 4, then stay no for everything after it."
  steps={[
    {
      label: 'Steps 1–3',
      title: 'Early steps run cleanly',
      body: 'Right tool, right arguments, a result that matches the question. Nothing here is the bug.',
      facts: [
        { label: 'Given everything before it, is this step correct?', value: 'Yes' },
      ],
    },
    {
      label: 'Step 4',
      title: 'The wrong record is fetched',
      body: 'Right tool, but an id from the wrong entity. In isolation it looks like an ordinary tool call.',
      facts: [
        { label: 'Given everything before it, is this step correct?', value: 'No — this is the bug' },
      ],
    },
    {
      label: 'Steps 5–11',
      title: 'Reasoning over the wrong record',
      body: 'The model reads the record it was handed and reasons about it well. Every one of these steps looks reasonable, because it is — the input is just wrong.',
      facts: [
        { label: 'Given everything before it, is this step correct?', value: 'Consequence, not cause' },
      ],
    },
    {
      label: 'Final answer',
      title: 'Where you noticed',
      body: 'Confident and well formatted, and traceable back to step 4 rather than to anything wrong with this step itself.',
      facts: [
        { label: 'Given everything before it, is this step correct?', value: 'Wrong — but not where it happened' },
      ],
    },
  ]}
/>
...
<Figure bare caption="The three ways a run goes wrong without ever raising, side by side. Only the rightmost column is fixed by touching the prompt — the other two live in the tool definitions and the tool itself, which is why 'the model is dumb' is rarely the right diagnosis.">
...
<Anatomy
  caption="One line of the record above, with its fields named. The step number matches the step-four example from Section 1 — the values are illustrative, not a real capture."
  label="One logged step"
  parts={[
    { text: '{\n  "run_id": ' },
    { text: '"a1b2c3"', name: 'run_id', note: 'Without it you cannot pull one run out of interleaved logs' },
    { text: ',\n  "step": ' },
    { text: '4', name: 'step', note: 'The order steps ran in — the axis you read along', subject: true },
    { text: ',\n  "kind": "tool", "name": "get_order"' , name: 'kind and name', note: 'Model call or tool call, and which one' },
    { text: ',\n  "input": {"order_id": "cus_492"}', name: 'input', note: 'The exact arguments, verbatim — this is where most bugs are visible' },
    { text: ',\n  "output": {"status": "not_found"}', name: 'output', note: 'The raw result the model saw, not a summary of it' },
    { text: ',\n  "ms": 812\n}' },
  ]}
/>
