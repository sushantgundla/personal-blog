...
**What field data shows:** harnesses that re-inject state blocks on every step see constraint-violation rates drop by 70-85% after step twenty, compared to systems that state constraints once at the start. The pattern that works is simple: task, boundaries (what not to do), current sub-goal, and facts discovered so far. Keep it under 200 tokens. Append it after tool results, not before — the API requires tool results first in the user turn, and recency matters.
