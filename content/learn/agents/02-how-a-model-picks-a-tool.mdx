...
**Prompt caching applies to it.** The tools array sits at the front and rarely changes, which makes it the ideal thing to cache across the steps of a long run. Changing `tool_choice` mid-run invalidates cached message content, so pick it and leave it alone. As of mid-2026, production systems that cache tool definitions across multi-step runs see 60-80% reductions in input token costs for runs longer than five steps. The break-even is around three steps for a typical tool array.
