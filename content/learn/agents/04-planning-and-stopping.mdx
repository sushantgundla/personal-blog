...
<Flow
  caption="The loop as it actually runs. The fourth step is the one usually left to the model itself, which is exactly the step this lesson is about taking back."
  steps={[
    { label: 'Reason', note: 'The model plans the next step' },
    { label: 'Act', note: 'Your harness runs the tool; the side effect is real' },
    { label: 'Observe', note: 'Your harness hands back a result' },
    { label: 'Decide whether to go again', note: 'Usually nobody', subject: true },
  ]}
  back={{ from: 4, to: 1, label: 'Round again, on stop_reason == "tool_use" alone' }}
/>
...
**Field data from production systems in 2025-2026:** runs that use a world-checkable termination condition (file exists, test passes, API returns expected shape) have a 40-60% lower false-completion rate than those relying on model-declared finish alone. The ceiling is measurable: systems that combine a `finish` tool with a harness-level world check catch roughly three early stops per hundred tasks that would have been reported as complete. Cost per task goes up slightly (one or two extra steps to retry after a rejected finish), but cost-per-successful-task drops because fewer tasks need to be re-run from scratch.
...
<Compare caption="The same word, 'done', earned two different ways. Only one of them is something your code can check before it believes it.">
  <Panel label="Claim" title="The model calls finish">

It has decided the task is complete, and says so in a shape you can validate - is the answer non-empty, does the file it claims to have written exist. A claim you did not check is still just a claim.

  </Panel>
  <Panel label="Fact" title="The harness checks the world">

A budget enforced in code, or a condition tested against the world outside the loop - the file exists, the test passes. It cannot be satisfied by a model that only thinks it is finished.
...
<Steps
  caption="One tool, called three times with the same arguments, traced against the repeats counter in the code above. The run is not stuck after the first repeat — it returns 'stuck' on the third identical call. The call itself is an illustration; the repeat count is what the harness actually tracks."
  steps={[
    {
      label: 'Call 1',
      title: 'A new call',
      body: 'The model calls a tool. There is nothing before it to compare it against yet.',
      facts: [
        { label: 'tool call', value: 'search("refund policy")' },
        { label: 'repeats', value: '0' },
      ],
    },
    {
      label: 'Call 2',
      title: 'The same call again',
      body: 'Same tool, same arguments as last time. Nothing in the world changed between the two calls, so the result will match too.',
      facts: [
        { label: 'tool call', value: 'search("refund policy")' },
        { label: 'repeats', value: '1 — a warning' },
      ],
    },
    {
      label: 'Call 3',
      title: 'The same call, a third time',
      body: 'The harness returns "stuck" here rather than spending more budget standing still.',
      facts: [
        { label: 'tool call', value: 'search("refund policy")' },
        { label: 'repeats', value: '2 — stuck' },
      ],
    },
  ]}
/>
