...
**Measured in production, 2025-2026:** median runs in coding agents hit 12-15 steps, research agents 8-12 steps, and customer-support agents 4-7 steps. The 95th percentile is roughly double the median in each category. Teams that set step budgets at the 95th percentile plus 20% headroom keep runaways under control without capping legitimate work. Those that set budgets based on gut feel ("20 sounds generous") either cut off real tasks early or let runaways burn hundreds of thousands of tokens before anyone notices.

<Steps
  caption="The same five rows as the table above, walked through one step at a time. Watch the second field, not the first — it is the running total that quadruples, not what any single step adds. These are the lesson's own illustrative round numbers, not a measurement — count your own for your task."
  steps={[
    {
      label: 'Step 1',
      body: 'The task, and nothing else yet.',
      facts: [
        { label: 'Input sent that step', value: '2k' },
        { label: 'Total input billed so far', value: '2k' },
      ],
    },
    {
      label: 'Step 5',
      body: 'Four rounds of tool calls and their results are now resent on every request.',
      facts: [
        { label: 'Input sent that step', value: '10k' },
        { label: 'Total input billed so far', value: '30k' },
      ],
    },
    {
      label: 'Step 10',
      body: 'The transcript has grown again, and so has what this step alone resends.',
      facts: [
        { label: 'Input sent that step', value: '20k' },
        { label: 'Total input billed so far', value: '110k' },
      ],
    },
    {
      label: 'Step 20',
      body: 'Twice the steps of step 10 — but the running total is closer to four times as large.',
      facts: [
        { label: 'Input sent that step', value: '40k' },
        { label: 'Total input billed so far', value: '420k' },
      ],
    },
    {
      label: 'Step 40',
      body: 'Twice the steps of step 20 again, and the running total has roughly quadrupled again with it.',
      facts: [
        { label: 'Input sent that step', value: '80k' },
        { label: 'Total input billed so far', value: '1.6m' },
      ],
    },
  ]}
/>
...
<Compare caption="The same oversized tool result, handled at two different points. Trimming only pays off when it happens before the append — trim after, and every remaining step has already inherited the full cost.">
  <Panel label="Naive" title="Append it verbatim, trim later">

The full page, table or trace goes straight into the transcript. Every step after this one resends it in full, whether the model needed all of it or not — trimming it afterwards no longer un-spends what earlier steps already paid.

  </Panel>
  <Panel label="What to do" title="Truncate at the executor, before it is appended">

The result is clipped to its head and tail — the tail is usually where the error message is — before it ever becomes part of the conversation. Every later step resends only the clipped version.
...
<Flow
  caption="The shape the code below runs. There are exactly two ways out — budget_exhausted or a real stop_reason — and the only way back to the top is a tool call that has not yet burned through either ceiling."
  steps={[
    { label: 'Send one request', note: 'The cached prefix, the tools, and the whole transcript so far' },
    { label: 'Add usage to spent', note: 'Input tokens plus any cache write, this step' },
    {
      label: 'spent vs the token budget',
      subject: true,
      branch: ['Over — stop and return budget_exhausted', 'Under — keep going'],
    },
    {
      label: 'stop_reason',
      branch: ['tool_use — dispatch it, append the result', 'anything else — stop and return the answer'],
    },
  ]}
  back={{ from: 4, to: 1, label: 'Round again on tool_use, until the budget or the step cap ends it' }}
/>
