...
**From the field:** teams shipping agents in 2025-2026 report that roughly 65% of tool failures in production are transient infrastructure issues (rate limits, timeouts, temporary service unavailability) that resolve on retry, 25% are legitimate "world said no" results (resource not found, permission denied, empty result set), and only 10% are schema violations or argument errors. Most harnesses handle the first bucket wrong by surfacing every retry to the model, which burns tokens on information the model cannot use. Handle transient failures under the tool; surface only what survives backoff.
