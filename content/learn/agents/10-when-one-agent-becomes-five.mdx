...
**What you gain.** Each worker gets a smaller, cleaner context — its own instruction, its own tools, its own transcript. A worker that only searches never sees the formatting rules, so the formatting rules never crowd out the search. That is the same effect lesson 6 described inside one run, applied across processes. And when subtasks do not depend on each other, they run at the same time, so a job that took six sequential minutes finishes in ninety seconds.
...
Both names appeared in lesson 1, in the list of workflow patterns, and that is not a contradiction — it is the same test doing its job. Lesson 1 asks who picks the next step, and the answer here is **whether the model chooses the decomposition**. When you enumerate the subtasks in your own code, this is a workflow that happens to run several contexts at once, and everything lesson 1 says about a drawable path and a known bill still holds. When an orchestrator reads the task and decides at run time how many workers there are and what each one is for, the same picture is an agent, and you have bought its unpredictability along with its flexibility.
...
Being blunt about it: **most multi-agent setups are not worth it.** They usually get built because a single agent was struggling, and a single agent struggling is far more often a tool-design problem, a context problem, or a missing step limit than a headcount problem. Fix those first — they are lessons 3, 6 and 4 of this course, they cost nothing extra to run, and they frequently make the split unnecessary.
