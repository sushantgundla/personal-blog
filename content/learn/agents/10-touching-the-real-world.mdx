...
<Figure bare caption="Read down a column, not across: once you know which of the three buckets a tool is in, its isolation and human-gate answers are already decided.">
...
<Compare caption="The same database, reached two ways. The narrow tool cannot be talked into anything but the one job it was given, whatever argument or injected instruction shows up in its input.">
  <Panel label="Naive" title="run_sql(query)">

One tool that can do everything the database can do. A bad argument or an injected instruction has the whole schema to work with.

  </Panel>
  <Panel label="What to do" title="get_orders_for_customer(customer_id)">

One tool that can do one thing. More work to write, and there is no query it can be handed that reaches outside one customer's orders.
...
<Plot
  caption="Cost and what a level stops rise together, with no shortcut — the second row, a subprocess under an OS sandbox, is the cheap default this lesson keeps pointing back to for anything running locally."
  xAxis="What it costs to run"
  xEnds={['No added cost', 'Hundreds of ms to boot']}
  yAxis="What it stops"
  yEnds={['Stops nothing', 'Host kernel access']}
  points={[
    { label: 'Same process', x: 3, y: 3 },
    { label: 'Subprocess + sandbox', x: 18, y: 42, subject: true },
    { label: 'Container', x: 45, y: 58 },
    { label: 'gVisor', x: 68, y: 82 },
    { label: 'microVM', x: 92, y: 97 },
  ]}
/>
...
**What production teaches:** the most common sandbox escape in 2025-2026 was not a kernel exploit or a container breakout. It was a tool that launched a subprocess without inheriting the parent's restrictions, or a network policy that allowed "just one API" which turned out to support webhooks to arbitrary URLs. Defense in depth means every layer — the policy check in your code, the sandbox around the process, the scoped credential, and the human gate on irreversible actions — assumes the layer before it will fail.
...
<Anatomy
  caption="The same three moments from the snippet above, named in place: resolution before the check, a result handed back instead of an exception, and the comment that admits what the line above it is not."
  label="Inside call_tool"
  parts={[
    {
      text: '    resolved = (WORKSPACE / candidate).resolve()\n    if not resolved.is_relative_to(WORKSPACE):',
      name: 'Resolve, then check',
      note: "The only order that catches '../..' and a symlink — both look fine before resolution.",
    },
    { text: '\n\n' },
    {
      text: '    if name in NEEDS_APPROVAL and not ask_human(name, args):\n        return {"ok": False, "error": "denied by operator"}',
      name: 'Denied, not raised',
      note: 'A tool result the model can read and act on, so the loop can pick something else.',
    },
    { text: '\n\n' },
    {
      text: '    return TOOLS[name](**args)                 # runs inside the sandbox, not here',
      name: 'The honest comment',
      note: 'Nothing above this line is a sandbox. It is the policy layer from section 2, and it is worth nothing on its own.',
      subject: true,
    },
  ]}
/>
