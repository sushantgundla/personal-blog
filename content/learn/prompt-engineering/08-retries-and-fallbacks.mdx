...
quiz:
  ...
  - q: "Nudging the temperature up on a retry helps because..."
    options: ["A higher temperature makes the model more capable", "At temperature 0 the same input tends to give the same output, so a blind retry reproduces the failure", "It lowers the cost per token"]
    answer: 1
    explain: "A little randomness is what lets the second attempt take a different path."
...
deeper:
  ...
  - { label: "Anthropic — errors and error codes", href: "https://docs.anthropic.com/en/api/errors" }
