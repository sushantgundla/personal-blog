...
deeper:
  - { label: "Anthropic — be clear and direct", href: "https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices#be-clear-and-direct" }
...
<Figure bare caption="The three causes produce the same symptom, a wrong answer, and diverge only in the middle column — which is why the fix always starts with naming which one you have, not with rewriting the prompt.">
...
| **Beyond the model** — the task is genuinely too hard as posed | Fails the same way every time, on a tiny clean input, however you word it. | Split it, give it a tool, or do that part in code. |
...
**Step three: only now, believe it is hard.** A task that fails on a tiny clean input, with an accurate restatement, and fails the same way on every attempt, is a task the model cannot do as posed. That is a real category — exact arithmetic on long numbers, anything needing information the model does not have, anything with five interacting sub-tasks. The fix is not a better sentence. It is decomposition (*Decomposition*), a tool, or code.

<Steps
  caption="The same order, one step at a time. Click through and watch what you're testing and what a failure there means change at every step — the expensive conclusion is reached only once the first two tests have passed."
  steps={[
    {
      label: 'Restate',
      title: 'Ask it to restate the task',
      body: 'Before it does the work, ask what it thinks the work is, and what could be read more than one way.',
      facts: [
        { label: 'you test', value: 'the restatement' },
        { label: 'a failure here means', value: 'ambiguous — describe the output' },
      ],
    },
    {
      label: 'Shrink',
      title: 'Re-run on a small, clean input',
      body: 'The restatement was right, but the answer is still wrong. Cut the context down to one small, clean example and re-run.',
      facts: [
        { label: 'you test', value: 'a tiny clean input' },
        { label: 'a failure here means', value: 'buried — fix placement and delimiters' },
      ],
    },
    {
      label: 'Believe it',
      title: 'Only now, believe it is hard',
      body: 'Fails the same way every time, on a tiny clean input, with an accurate restatement, however you word it.',
      facts: [
        { label: 'you test', value: 'nothing left to test' },
        { label: 'the conclusion', value: 'genuinely hard — split it or use a tool' },
      ],
    },
  ]}
/>
...
<Compare caption="The same instruction, rewritten so the output is checkable instead of implied. The rewrite does not ask for more effort — it names an artefact the model can actually produce.">
  <Panel label="Weak" title="Please be very careful and accurate.">

Reads as pressure. It does not describe anything different for the model to produce, so there is nothing for the instruction to change.

  </Panel>
  <Panel label="Rewrite" title="Quote the exact sentence each value came from. If you cannot quote it, return null for that field.">

Care becomes a visible artefact you can check. The model either produces the quote or admits it cannot, and both are checkable.
...
**"Take your time" is not the same as chain-of-thought.** Asking for step-by-step reasoning does work, for a mechanical reason covered in *Thinking out loud* — the model gets more tokens to compute in. "Take your time" produces no extra tokens, so it produces no extra computation. The phrasing is similar; the effect is not.
