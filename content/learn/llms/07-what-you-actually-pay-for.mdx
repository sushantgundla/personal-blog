...
**Lever three: prompt caching.** If a long prefix repeats across requests, cache it. It cuts latency as well as cost, and it takes an afternoon. The rules that make it work are in *Context windows and their limits*.
...
<Flow
  caption="The three levers, tried in the order they pay off. Context is pulled first because it is both the biggest saving and the one least likely to cost you anything in accuracy."
  steps={[
    { label: 'Shorten the context', note: 'Trim the system prompt, prune tool schemas, retrieve instead of paste', subject: true },
    { label: 'Send it to a smaller model', note: 'Easy requests rarely need the frontier model' },
    { label: 'Cache the repeated prefix', note: 'Cuts latency as well as cost, for a prefix that repeats' },
  ]}
/>
...
<Scale
  caption="Three of the four culprits above, placed on a log rule by roughly how many tokens each one adds to a single request. The jump from a short message to a stuffed retrieval is on the order of a thousand times, before a single price is even applied. The figures are an illustration, not a measurement — count your own."
  unit="tokens"
  fromLabel="A short message"
  toLabel="A stuffed request"
  marks={[
    { at: 40, label: 'A short user message' },
    { at: 1200, label: 'A fat system prompt' },
    { at: 9000, label: 'Resent conversation history' },
    { at: 62000, label: 'A stuffed retrieval', subject: true },
  ]}
/>
...
RATES = {"claude-sonnet-4-20250514": {"in": 0.0, "out": 0.0}}

def ask(feature: str, prompt: str, model: str = "claude-sonnet-4-20250514") -> str:
...
<Anatomy
  caption="The one line every request logs. feature is the field worth marking — group by it once and the call costing ten times what you assumed finds itself."
  label="One log line"
  parts={[
    { text: 'llm feature=' },
    { text: '%s', name: 'feature', note: 'What you group cost by', subject: true },
    { text: ' model=' },
    { text: '%s', name: 'model' },
    { text: ' in=' },
    { text: '%d', name: 'input tokens' },
    { text: ' out=' },
    { text: '%d', name: 'output tokens' },
    { text: ' cost=' },
    { text: '%.5f', name: 'cost, computed from the usage object' },
    { text: ' secs=' },
    { text: '%.2f', name: 'latency' },
  ]}
/>
