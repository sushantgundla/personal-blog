...
<Flow
  caption="The decision behind the status table above. Most outcomes end the loop immediately — only the retryable ones earn a backoff and another attempt, and even those are capped rather than open-ended."
  steps={[
    { label: 'Request sent' },
    {
      ...
      subject: true,
      branch: [
        '400 / 401 / 403 — never retry, fix the request',
        '429 / 500 / 503 / 529 — retry after a backoff',
        'Timeout or reset — retry only if the call is safe to repeat',
      ],
    },
    { label: 'Exponential backoff, full jitter', note: 'Wait a random duration between zero and the ceiling' },
  ]}
  back={{ from: 3, to: 1, label: 'Try again, until the attempt cap or the total wait cap is hit' }}
/>

<Dial
  caption="The wait ceiling from the retry loop's own formula, min(60.0, 2.0 ** attempt), attempt by attempt. The actual wait is still random.uniform(0, ceiling), so two clients on the same attempt still land at different moments."
  name="attempt"
  min={0}
  max={6}
  step={1}
  start={0}
  bands={[
    { at: 0, label: 'First retry', outcome: 'Ceiling is min(60, 2**0) — one second.' },
    { at: 3, label: 'A few attempts in', outcome: 'Ceiling is min(60, 2**3) — eight seconds.' },
    { at: 6, label: 'Capped', outcome: '2**6 is 64, but the ceiling stopped climbing at 60 — the min(60.0, ...) in the code.', note: 'Past here, more attempts spend the retry budget without buying more patience.' },
  ]}
/>
...
<Compare caption="The same answer, delivered two ways. The total generation time is identical — what changes is when the user sees anything at all, and what a dropped connection costs you.">
  <Panel label="Without streaming" title="Wait for the whole answer">

The connection sits open for the entire generation. The user watches a blank screen until the last token is ready, and a long answer risks the HTTP-level timeout that streaming exists to avoid.

  </Panel>
  <Panel label="With streaming" title="Words arrive as they're produced">

The first tokens appear after about a second instead of forty. The user can watch it "thinking" instead of wondering if it broke, and you can abort the call cleanly the moment they navigate away.
...
def ask(prompt: str, model: str = "claude-sonnet-4-20250514", attempts: int = 5) -> str:
