...
<Compare caption="The same permission fact, stored two ways. The per-user list has to be rewritten on every membership change with nothing to trigger the rewrite; the per-group list only needs the user's current groups resolved fresh, on the query that actually needs them.">
  <Panel label="Naive" title="A list of user IDs, written at ingestion">

Works on day one. Rots from day two, because it has to be rewritten every time group membership changes anywhere near the document, and nothing tells you that happened.

  </Panel>
  <Panel label="What to do" title="A list of group IDs, resolved against the user live">

The chunk stores the group or role it belongs to, written once. The user's current groups come from the identity provider on every query, so the part that changes constantly is never baked into the index.
...
<Flow
  caption="The check the code above runs on every query: the chunk's stored group list was written once, at ingestion, and everything else is resolved live, so revoking a user's group membership takes effect on their very next query."
  steps={[
    { label: 'Ingestion', note: 'allowed_group_ids and visibility written onto the chunk, once' },
    { label: 'A query arrives', note: "The user's current groups are resolved live, from the identity provider" },
    {
      label: 'is_visible check',
      subject: true,
      branch: [
        'visibility is "org" — visible to everyone',
        'otherwise — visible only if the user\'s groups intersect allowed_group_ids',
      ],
    },
  ]}
/>
...
<Steps
  caption="The test described above, one state at a time. Click through and watch whether the search result actually flips before the clock the stated freshness target promised runs out."
  steps={[
    {
      label: 'Revoke access',
      title: 'In the source system',
      body: "The test user is pulled from the folder. The source system's own sharing dialog shows them gone immediately.",
      facts: [
        { label: 'source system', value: 'access removed' },
        { label: 'search index', value: 'still holds the old allowed_group_ids' },
      ],
    },
    {
      label: 'Wait it out',
      title: 'The stated freshness target elapses',
      body: 'However long was promised, and no longer — the test only means something if it matches the claim written down.',
      facts: [
        { label: 'source system', value: 'access removed' },
        { label: 'search index', value: 'should now be synced, if the job ran' },
      ],
    },
    {
      label: 'Search as that user',
      title: 'The only check that counts',
      body: 'Not reading the sync code and assuming it works — actually querying as the revoked user.',
      facts: [
        { label: 'source system', value: 'access removed' },
        { label: 'search index', value: 'correct only if the document is now withheld' },
      ],
    },
    {
      label: 'Read the result',
      title: 'Pass or fail',
      body: 'The document still returning means there is no working sync job, whatever the code appears to do.',
      facts: [
        { label: 'document returned', value: 'fail — the sync has never been checked' },
        { label: 'document withheld', value: 'pass — the sync is confirmed, not assumed' },
      ],
    },
  ]}
/>
