...
<Compare caption="Same constraint, two different places to apply it. Only the query on the right guarantees the k you asked for — post-filtering fails silently, with no error telling you the count came back short.">
  <Panel label="Naive" title="Post-filter: search, then discard">

Ask the store for the top k, then throw away rows that fail the constraint in your own code. The store already picked its k without knowing the constraint exists, so anything the filter removes afterwards is simply gone.

  </Panel>
  <Panel label="What to do" title="Pre-filter or filtered search: constrain first">

Push the constraint into the query itself, next to the `ORDER BY`. The store decides how to combine it with the index, and the count you asked for is the count you get.
...
<Dial
  caption="The decay term from the query above, felt as a knob. The bands are illustrative — the half-life and the 0.15 weight are yours to tune against real queries, not fixed here."
  name="chunk age"
  min={0}
  max={730}
  step={10}
  ...
  unit="days"
  bands={[
    { at: 0, label: 'Fresh', outcome: 'No decay penalty yet — similarity alone decides the ranking.' },
    { at: 90, label: 'A few months old', outcome: 'The penalty has grown enough that a fresher, slightly weaker match can start to outrank it.' },
    { at: 180, label: 'Around the curve above', outcome: 'Well past the early climb — an old chunk needs a clear similarity edge to still win.' },
    { at: 365, label: 'A year old', outcome: 'Close to the full 0.15 penalty — only a chunk that is clearly the best match survives being this old.' },
  ]}
/>
...
<Flow
  caption="What happens after a chunk is retrieved. Nothing downstream can undo it — the enforcement branch has to be decided before the query runs, not after."
  steps={[
    { label: 'Retrieval query runs', note: 'Against the store, with whatever constraints it was given' },
    {
      label: 'The permission check',
      subject: true,
      branch: [
        'Enforced by the store — only permitted chunks come back',
        'Left to a prompt instruction — the chunk comes back regardless',
      ],
    },
    { label: 'Whatever comes back is exposed', note: 'In the context window, usually logged, and often shown in a citations panel' },
  ]}
/>
