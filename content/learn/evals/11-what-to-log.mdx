...
<Compare caption="The rule the previous lesson left you with, and the scope this one puts on it. Structure and timing are still logged exactly as before — the change is a second pass that decides, per field, what a value is before it is written.">
  <Panel label="Naive" title="Log inputs and outputs, verbatim">

Every field a span could hold is written exactly as it arrived — a prompt, a tool argument, a model's response. It looks complete, right up until one of those fields is a user's own SSN, pasted into a chat and asked to be confirmed, now sitting in plain text in a trace viewer.

  </Panel>
  <Panel label="What to do" title="Verbatim, minus what shouldn't be kept">

Structure, timing and token counts are still logged unconditionally. Free text a user controls is redacted or hashed before the write. A secret is never captured in the first place, so there is nothing to redact later.
...
<Flow
  caption="Every field is sorted into one of three buckets before anything is written, never after — sorting it out once millions of rows already exist is the much worse version of this same job."
  steps={[
    { label: 'A field a span could capture', note: 'Content, timing, an identifier, a secret' },
    {
      label: 'Sorted into a bucket, at instrumentation time',
      subject: true,
      branch: [
        'Always log — structure, timing, token counts, model id, status',
        'Redact or hash — free text a user controls',
        'Never log, no exception — secrets: keys, tokens, credentials',
      ],
    },
  ]}
/>
...
<Dial
  caption="One rate, three regions of exposure. The bands are illustrative, not a recommended setting — weight your own slice toward errors, low eval scores and anything flagged for review, whatever percentage that turns out to be."
  name="percent of runs captured at full, redacted detail"
  min={0}
  max={100}
  step={5}
  start={15}
  ...
  bands={[
    {
      ...
      outcome: 'Every run is traced for timing, status and token counts. Cheap, and none of it is the risk this lesson is about.',
    },
    {
      at: 15,
      label: 'Weighted sample',
      outcome: 'Full redacted content is captured only for the runs worth debugging — errors, low eval scores, anything flagged for review.',
    },
    {
      at: 100,
      label: 'Everything, always',
      outcome: 'A complete archive of full, redacted content for every run — more exposure than a redaction step was ever built to carry, for no debugging gain over the weighted sample.',
    },
  ]}
/>
