...
<Figure bare caption="Three ways to source a case, laid out on the same three questions. Traffic-sourced is cheapest precisely because the input already exists — labeling it is the only work left, which is why it should make up the largest share of the set.">
...
<Flow
  caption="The one new step in an incident review: once the cause is understood, it becomes a case that stays in the set for good, not a note that gets closed along with the ticket."
  steps={[
    { label: 'A production incident', note: 'A wrong answer, a policy violation, an escalation' },
    { label: 'Someone investigates', note: 'Already happening - no extra process to add here' },
    { label: 'Write the input and correct output down', subject: true, note: 'The one new step: turn what was found into a case' },
    { label: 'Add it to the set, permanently', note: 'Not until it is fixed - forever, like a regression test' },
  ]}
/>
...
<Compare caption="The same generated candidate, handled two ways. Only one of them can tell you whether the model's idea of a hard input matches your users' reality.">
  <Panel label="Unchecked" title="Straight into the set">

The candidate ships untouched. If the model's idea of a hard input misses a real blind spot, that blind spot is now invisible - baked into a set the team trusts.

  </Panel>
  <Panel label="What to do" title="A person reads it first">

Someone checks the input is realistic and writes the outcome they would actually accept, the same as with a traffic-sourced case, before it goes anywhere near the set.
...
<Steps
  caption="One eval set's first few weeks, illustrated rather than measured - count your own cases the same three ways: traffic, incident and synthetic-reviewed."
  steps={[
    {
      label: 'Day one',
      title: 'A first pass from the logs',
      body: 'A few dozen real, checked cases, pulled straight from traffic and support tickets.',
      facts: [
        { label: 'Traffic-sourced', value: '30' },
        { label: 'Incident-based', value: '0' },
        { label: 'Synthetic, reviewed', value: '0' },
      ],
    },
    {
      label: 'First incident',
      title: 'A failure becomes a case',
      body: 'Production breaks once, someone investigates, and the case goes in for good.',
      facts: [
        { label: 'Traffic-sourced', value: '30' },
        { label: 'Incident-based', value: '1' },
        { label: 'Synthetic, reviewed', value: '0' },
      ],
    },
    {
      label: 'A known gap',
      title: 'Synthetic cases fill it, checked first',
      body: 'A feature nobody has used yet gets a handful of reviewed, model-generated cases to cover the hole traffic has not made yet.',
      facts: [
        { label: 'Traffic-sourced', value: '30' },
        { label: 'Incident-based', value: '1' },
        { label: 'Synthetic, reviewed', value: '6' },
      ],
    },
  ]}
/>
